...
$socioStmt = $pdo->prepare('SELECT nombre_completo, telefono, numero_polla, periodicidad_pago, valor_presupuestado FROM socios WHERE id_socio = :id');
$socioStmt->execute([':id' => $idSocio]);
$socio = $socioStmt->fetch();
if (!$socio) {
    exit('Socio no encontrado.');
}

$where = ['m.id_socio = :s'];
$params = [':s' => $idSocio];
if ($desde) { $where[] = 'm.fecha >= :d'; $params[':d'] = $desde; }
if ($hasta) { $where[] = 'm.fecha <= :h'; $params[':h'] = $hasta; }
if ($actividad) { $where[] = 'm.id_actividad = :a'; $params[':a'] = $actividad; }
