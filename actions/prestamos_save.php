...
if ($accion === 'eliminar') {
    try {
        ...
        $montoDesembolso = (float) ($prestamo['monto_prestamo'] ?? 0);
...
$saldoCapital = $monto;
$saldoInteres = 0;
$montoDesembolso = $monto;
$montoEntregado = max(0, $monto - $interesMensual);
...
$observacionMovimiento = sprintf(
    '%s. Entrega neta: $%s (interés anticipado $%s)',
    $observacionBase,
    number_format($montoEntregado, 0, ',', '.'),
    number_format($interesMensual, 0, ',', '.')
);
